...
const WithdrawalsScreen: React.FC<WithdrawalsScreenProps> = ({ withdrawals, setWithdrawals }) => {
  ...
  const filteredWithdrawals = useMemo(() => {
    return withdrawals.filter(req => {
      ...
      return matchesTab && (!search || req.userName.toLowerCase().includes(search) || req.userId.toLowerCase().includes(search));
    });
  }, [activeTab, searchTerm, withdrawals]);

  const handleApprove = (id: string) => {
    const now = new Date().toLocaleString();
    setWithdrawals(prev => prev.map(w => w.id === id ? { ...w, status: 'approved', processedAt: now } : w));
  };

  const handleReject = (id: string, reason: string) => {
    const now = new Date().toLocaleString();
    setWithdrawals(prev => prev.map(w => w.id === id ? { ...w, status: 'rejected', rejectionReason: reason, processedAt: now } : w));
  };
